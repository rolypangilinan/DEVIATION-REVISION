#%%
# AI
# WITH AI 200 ROWS AVERAGE
# WITH AI FLUCTUATION LOG
# WITH AI REAL TIME UPDATE
# ERROR: 3 COPY DISPLAY OF FLUCTUATION LOG


# FIXED: WITH FUTURE WARNING 
# COMBINED 2 PROGRAM (1 DEVIATION1B.PY AND 2 LTGHis_27.PY) - I WANT YOU TO COPY ALL TKINTER AT CODE 2 TO CODE 1 AND ALSO THE LOGICAL COMPUTATION
...
file_path = r"\\192.168.2.19\ai_team\AI Program\Outputs\CompiledPiMachine\CompiledPIMachine.csv" # FC1
# file_path = r"C:\Users\ai.pc\OneDrive\Desktop\CompiledPIMachine.csv" # TESTING
...
for model, group in compiledFrame.groupby('MODEL CODE'):
    ...
    for date in past_data['DATE'].dt.date.unique():
        ...
        if accumulated_rows.empty:
            accumulated_rows = valid_rows.copy()
        else:
...
class FluctuationMonitor:
    def __init__(self, root, location, threshold=3.0, generate_csv="NO", model="60CAT0213P", selected_measurements=None):
        ...
        ttk.Button(button_frame, text="SIMULATION", command=self.simulation).pack(side=tk.LEFT, padx=2)
...
# AI-BASED AVERAGE COMPUTATION
    def compute_ai_average(self, series):
        n = len(series)
        if n < 5:
            return series.mean() if n > 0 else 0.0
    ...
    def update_spc_graphs(self):
        ...
        try:
            ...
            for meas in self.mid_measurements:
                full_data = df[meas]
                if pd.isna(full_data).all():
                    continue
                df_prev = df[df['DATETIME'].dt.date < datetime.now().date()]
                full_data_prev = df_prev[meas].tail(200)
                mean_val = full_data_prev.mean()
                self.avg_labels[meas].config(text=f"{mean_val:.2f}")
                ...
                if self.tick_strategy == "last":
                    ...
                    if self.x_axis_mode.get() == "Numerical":
                        ...
                        tick_labels = df['DATETIME'].tail(tail_size).iloc[tick_rel_pos].dt.strftime('%Y-%m-%d %H:%M:%S')
                    else:
                ...
                is_fluctuated = df[fluct_col].iloc[-1] > self.threshold if fluct_col in df.columns else False
                ...
                self.canvases[meas].draw()
                self.ucl_labels[meas].config(text=f"{upper_limit:.2f}")
                self.lcl_labels[meas].config(text=f"{lower_limit:.2f}")
    ...
    def process_and_update(self):
        ...
        try:
            ...
            if not past_data.empty:
                ...
                if not group.empty:
                    accumulated_rows = group.sort_values('DATETIME').tail(200)
            ...
            for _, row in new_df.iterrows():
                ...
                ave_v_max = self.compute_ai_average(previous_pass["VOLTAGE MAX (V)"]) if not previous_pass.empty else 0
                ave_w_max = self.compute_ai_average(previous_pass["WATTAGE MAX (W)"]) if not previous_pass.empty else 0
                ave_cp_max = self.compute_ai_average(previous_pass["CLOSED PRESSURE_MAX (kPa)"]) if not previous_pass.empty else 0
                ave_v_mid = self.compute_ai_average(previous_pass["VOLTAGE Middle (V)"]) if not previous_pass.empty else 0
                ave_w_mid = self.compute_ai_average(previous_pass["WATTAGE Middle (W)"]) if not previous_pass.empty else 0
                ave_a_mid = self.compute_ai_average(previous_pass["AMPERAGE Middle (A)"]) if not previous_pass.empty else 0
                ave_cp_mid = self.compute_ai_average(previous_pass["CLOSED PRESSURE Middle (kPa)"]) if not previous_pass.empty else 0
                ave_v_min = self.compute_ai_average(previous_pass["VOLTAGE MIN (V)"]) if not previous_pass.empty else 0
                ave_w_min = self.compute_ai_average(previous_pass["WATTAGE MIN (W)"]) if not previous_pass.empty else 0
                ave_cp_min = self.compute_ai_average(previous_pass["CLOSED PRESSURE MIN (kPa)"]) if not previous_pass.empty else 0
            ...
            if dataList:
                ...
                for col in emptyColumn:
                    if col not in new_data.columns:
                        new_data[col] = np.nan
                concat_df = pd.concat([self.compiledFrame, new_data]) if not self.compiledFrame.empty else new_data
                self.compiledFrame = concat_df.drop_duplicates(subset=['S/N', 'DATETIME'], keep='last').sort_values('DATETIME').reset_index(drop=True)
                self.compiledFrame = self.compiledFrame[self.compiledFrame['PASS/NG'] == 1].reset_index(drop=True)
            ...
            for model in unique_models:
                ...
                accumulated_rows = pd.DataFrame()
                count = 0
                for date in group['DATE'].dt.date.unique():
                    daily_rows = group[group['DATE'].dt.date == date]
                    valid_rows = daily_rows[daily_rows["PASS/NG"] == 1]
                    if accumulated_rows.empty:
                        accumulated_rows = valid_rows.copy()
                    else:
...
# 3RD TKINTER FOCUS
    ...
    def simulation(self):
        ...
        text_frame = ttk.Frame(sim_window)
        text_frame.pack(fill=tk.BOTH, expand=True)
        text = tk.Text(text_frame)
        text.pack(fill=tk.BOTH, expand=True)
        pie_frame = ttk.Frame(sim_window)
        pie_frame.pack(fill=tk.BOTH, expand=True)
        df = self.compiledFrame.copy()
        if self.current_model:
            df = df[df['MODEL CODE'] == self.current_model]
        df['DATE'] = pd.to_datetime(df['DATE'], format='%Y/%m/%d')
        today = datetime(2025,10,15).date()
        df_prev = df[df['DATE'].dt.date < today]
        last_200 = df_prev.sort_values('DATETIME').tail(200)
        if len(last_200) < 200:
            text.insert(tk.END, "Not enough data for 200 rows\n")
        else:
            start_date = last_200['DATE'].dt.date.max()
            end_date = last_200['DATE'].dt.date.min()
            group = last_200.groupby(last_200['DATE'].dt.date).size()
            sorted_dates = sorted(group.keys(), reverse=True)
            total_prev = len(df_prev)
            left_rows = total_prev - 200
            before_200 = df_prev.sort_values('DATETIME').head(left_rows)
            last_non_date = before_200['DATE'].dt.date.max() if left_rows > 0 else None
            remaining = before_200[before_200['DATE'].dt.date == end_date].shape[0] if last_non_date == end_date else 0
            for i, d in enumerate(sorted_dates):
                collected = group[d]
                if i == 0:
                    text.insert(tk.END, f"START DATE: {d} = {collected} ROWS COLLECTED, NO REMAINING, TOTAL OF {collected} ROWS\n")
                elif i == len(sorted_dates) - 1:
                    total_on_date = collected + remaining
                    rem_str = f"{remaining} ROWS REMAINING" if remaining > 0 else "NO REMAINING"
                    text.insert(tk.END, f"END DATE: {d} = {collected} ROWS COLLECTED, {rem_str}, TOTAL OF {total_on_date} ROWS\n")
                else:
                    text.insert(tk.END, f"{d} = {collected} ROWS COLLECTED, NO REMAINING, TOTAL OF {collected} ROWS\n")
            for meas in self.mid_measurements:
                data = last_200[meas]
                mean = data.mean()
                ucl = mean * (1 + self.threshold)
                lcl = mean * (1 - self.threshold)
                displayed_avg = float(self.avg_labels[meas].cget("text"))
                displayed_ucl = float(self.ucl_labels[meas].cget("text"))
                displayed_lcl = float(self.lcl_labels[meas].cget("text"))
                displayed_start = self.start_labels[meas].cget("text")
                displayed_end = self.end_labels[meas].cget("text")
                comp_start = last_200.iloc[-1]['DATE'].strftime('%Y-%m-%d')
                comp_end = last_200.iloc[0]['DATE'].strftime('%Y-%m-%d')
                text.insert(tk.END, f"\n{meas}:\n")
                text.insert(tk.END, f"AVERAGE OF 200 ROWS: {mean:.2f} {'✅' if abs(mean - displayed_avg) < 0.01 else '❌'}\n")
                text.insert(tk.END, f"UCL: {ucl:.2f} {'✅' if abs(ucl - displayed_ucl) < 0.01 else '❌'}\n")
                text.insert(tk.END, f"LCL: {lcl:.2f} {'✅' if abs(lcl - displayed_lcl) < 0.01 else '❌'}\n")
                text.insert(tk.END, f"START: {comp_start} {'✅' if comp_start in displayed_start else '❌'}\n")
                text.insert(tk.END, f"END: {comp_end} {'✅' if comp_end in displayed_end else '❌'}\n")
                text.insert(tk.END, f"HOW MANY ROWS COLLECTED: {len(last_200)} {'✅' if len(last_200) == 200 else '❌'}\n")
                # text.insert(tk.END, f"COUNTING OF ROWS PER DAY, START WITH START DATE AND END WITH END DATE\n")
        full_df = pd.read_csv(self.file_path, encoding='latin1', dtype={'S/N': str, 'MODEL CODE': str})
        if self.current_model:
            full_df = full_df[full_df['MODEL CODE'] == self.current_model]
        full_df['DATE'] = pd.to_datetime(full_df['DATE'], format='%Y/%m/%d')
        yesterday = today - timedelta(days=1)
        yest_df = full_df[full_df['DATE'].dt.date == yesterday]
        today_df = full_df[full_df['DATE'].dt.date == today]
        for period, p_df in [('Yesterday', yest_df), ('Today', today_df)]:
            total = len(p_df)
            if total > 0:
                pass_count = sum(p_df['PASS/NG'] == 1)
                ng_count = total - pass_count
                fig = Figure(figsize=(5,5))
                ax = fig.add_subplot(111)
                ax.pie([pass_count, ng_count], labels=['PASS', 'NG'], autopct='%1.1f%%', colors=['green', 'red'])
                ax.set_title(period)
                canvas = FigureCanvasTkAgg(fig, master=pie_frame)
                canvas.draw()
                canvas.get_tk_widget().pack(side=tk.LEFT, padx=10)
            else:
                label = ttk.Label(pie_frame, text=f"No data for {period}")
