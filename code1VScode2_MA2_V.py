...
class DatabaseSelection:
    ...
    def confirm_selection(self):
        ...
        if location:
            self.root.destroy()
            root = tk.Tk()
            app = FluctuationMonitor(root, location, threshold, generate_csv, model)
...
class FluctuationMonitor:
    def __init__(self, root, location, threshold=3.0, generate_csv="YES", model="60CAT0213P"):
        self.root = root
...
# 3RD TKINTER FOCUS
    ...
    def add_to_log(self, row, count):
        ...
        for name, (value_col, ave_col, dev_col, fluct_col) in self.measurements_map.items():
            if abs(row[fluct_col]) > self.threshold:
                ave = row[ave_col]
                dev = row[dev_col] * 100
