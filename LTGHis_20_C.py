...
class FluctuationMonitor:
    ...
    def update_spc_graphs(self):
        ...
        try:
            ...
            for meas in self.mid_measurements:
                ...
                self.avg_labels[meas].config(text=f"{mean_val:.6f}")
                ...
                self.canvases[meas].draw()
                self.ucl_labels[meas].config(text=f"{upper_limit:.6f}")
                self.lcl_labels[meas].config(text=f"{lower_limit:.6f}")
                self.message_labels[meas].config(text="FLUCTUATED" if is_fluctuated else "")
                self.last_labels[meas].config(text=f"{last_y:.6f}")
        except Exception as e:
    ...
    def process_and_update(self):
        ...
        try:
            ...
            new_df = df.iloc[self.last_row_count:]
...
# 3RD TKINTER FOCUS
    ...
    def open_computation_window(self):
        ...
        for meas in self.mid_measurements:
            ...
            tree.insert("", "end", values=(proc_name, f"{mean_val:.6f}", f"{plus_tol:.6f}", f"{minus_tol:.6f}", f"{ucl:.6f}", f"{lcl:.6f}"))
