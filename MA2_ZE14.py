...
# PUT PROCESS INSPECTION VALUES AT THE TOP OF UCL
# CREATE A "COMPUTATION" BUTTON AFTER X-AXIS BUTTON
# FIXED: ERROR: GENERATE CSV FILE (YES) BUT NOT CREATING CSV FILE
# NOT YET TRIED TO RUN AT FC1
# NOT YET TESTED ALL DATA
# READ THE SERIAL NO EVENTHOUGH IT WAS REPEATED TWICE
# ERROR: SERIAL 250901045 ALREADY LOG IN AT LOG FILES FOR FLUCTUATION(REPEATED SERIAL NO. DOESN'T READ BY THE PROGRAM)
# ERROR: GENERATE CSV FILE (YES) BUT NOT CREATING CSV FILE
# TESTED AT FC1 
# NEED TO RESTART PROGRAM FOR CHANGE MODEL
# DONE TESTING ALL DATA
# WITH LOG BUT NOT UPDATING AT NOTEPAD
# WITH REAL TIME UPDATING
# WITH PROCESS INSPECTION SELECTION (WORKING)
# UCL LCL
# WITHOUT SCROLLBAR AT 4 LINE TREND GRAPH
...
class DatabaseSelection:
    def __init__(self, root):
        ...
        self.root.geometry(f"{int(self.root.winfo_screenwidth() * 0.3)}x{int(self.root.winfo_screenheight() * 0.6)}") # ADJUST THE WINDOW HEIGHT OF THE 1ST TKINTER GUI
...
class FluctuationMonitor:
    def __init__(self, root, location, threshold=3.0, generate_csv="NO", model="60CAT0213P", selected_measurements=None):
        ...
        ttk.Label(self.left_frame, text="MID Measurements", font=('Arial', 12, 'bold')).pack(anchor='w', pady=(10,0))
        mid_fluct_cols = [self.measurements_map[m][3] for m in self.section_map["MID Measurements"] if m in self.selected_measurements]
        for col in mid_fluct_cols:
            self.create_status_row(self.left_frame, col)
        ...
        for title in ["MAX Measurements", "MIN Measurements"]:
    ...
    #        self.line_ax.plot([], [], label=measurement, color=self.colors[idx % len(self.colors)], linewidth=1.5)
    ...
    #                if n > 1:
    #                    self.line_ax.plot(x[:-1], y[:-1], color=color, linewidth=1.5, label=measurement if idx == 0 else "")
    #                    self.line_ax.plot(x[-2:], y[-2:], color=last_color, linewidth=1.5, linestyle='--', label=measurement if idx == 0 else "")
    #                else:
    #                    self.line_ax.plot(x, y, color=color, linewidth=1.5, label=measurement if idx == 0 else "")
    #                last_x = x[-1]
    #                last_y = y[-1]
    #                circle_color = 'red' if is_fluctuated else 'none'
    #                self.line_ax.scatter(last_x, last_y, color=circle_color, s=100, marker='o', zorder=5, linewidth=2, edgecolors='black' if is_fluctuated else 'none')
    ...
    #                    self.line_ax.set_xlabel('Sample Number')
    #                else:
    #                    tick_labels = df['DATETIME'].iloc[tick_positions].dt.strftime('%Y-%m-%d %H:%M:%S')
    #                    self.line_ax.set_xlabel('DateTime')
    ...
    def update_spc_graphs(self):
        ...
        try:
            ...
            for meas in self.mid_measurements:
                ...
                full_tick_positions = np.linspace(0, len(data)-1, 10, dtype=int)
                if self.tick_strategy == "last":
                    tick_positions = full_tick_positions[-self.num_ticks:]
                else:
    ...
    #    try:
    #        self.ax.clear()
    #        self.fluctuation_values = [last_row[self.dev_map[short]] * 100 for short in self.fluctuation_measurements]
    #        self.bars = self.ax.bar(self.fluctuation_measurements, self.fluctuation_values, color='skyblue')
    #        self.ax.set_title('Current Deviation Values')
    #        self.ax.set_ylabel('Fluctuation Amount (%)')
    #        self.ax.grid(True)
    #        self.ax.set_xticks(range(len(self.fluctuation_measurements)))
    #        self.ax.set_xticklabels(self.fluctuation_measurements, rotation=45, ha='right', fontsize=7)
    #        max_value = max([abs(v) for v in self.fluctuation_values]) if self.fluctuation_values and max([abs(v) for v in self.fluctuation_values]) > 0 else 10
    ...
    def process_and_update(self):
        ...
        try:
            ...
            if not past_compiled.empty:
                past_compiled['DATE'] = pd.to_datetime(past_compiled['DATE'], errors='coerce')
                ...
                if not group.empty:
                    ...
                    if dates:
                        ...
                        for date in dates:
                            ...
                            if count >= 200 and start_date is None:
                                start_date = date
                            if count >= 200:
                                break
                        if count < 200:
...
# 3RD TKINTER FOCUS
    def open_focus_window(self):
        ...
        self.focus_window.title("Focus on Historical Trends")
    ...
    def open_display_window(self):
        ...
        ttk.Label(self.display_window, text="X-AXIS:", font=('Arial', 12)).pack(pady=10)
        ttk.Radiobutton(self.display_window, text="Numerical Numbering", variable=self.x_axis_mode, value="Numerical").pack(anchor='w', padx=20)
        ttk.Radiobutton(self.display_window, text="DateTime", variable=self.x_axis_mode, value="DateTime").pack(anchor='w', padx=20)
        ttk.Radiobutton(self.display_window, text="None", variable=self.x_axis_mode, value="NONE").pack(anchor='w', padx=20)
        ttk.Checkbutton(self.display_window, text="Show Title", variable=self.show_title_var).pack(anchor='w', padx=20)
        ttk.Label(self.display_window, text="LAYOUT:", font=('Arial', 12)).pack(pady=10)
        ttk.Radiobutton(self.display_window, text="Vertical", variable=self.layout_mode, value="VERTICAL").pack(anchor='w', padx=20)
        ttk.Radiobutton(self.display_window, text="Horizontal", variable=self.layout_mode, value="HORIZONTAL").pack(anchor='w', padx=20)
        ttk.Label(self.display_window, text="SPC LINE TREND WIDTH:", font=('Arial', 12)).pack(pady=10)
        ttk.Radiobutton(self.display_window, text="EXTRA SMALL", variable=self.line_width_var, value="EXTRA SMALL").pack(anchor='w', padx=20)
        ttk.Radiobutton(self.display_window, text="SMALL", variable=self.line_width_var, value="SMALL").pack(anchor='w', padx=20)
        ttk.Radiobutton(self.display_window, text="MEDIUM", variable=self.line_width_var, value="MEDIUM").pack(anchor='w', padx=20)
        ttk.Radiobutton(self.display_window, text="LARGE", variable=self.line_width_var, value="LARGE").pack(anchor='w', padx=20)
        ttk.Radiobutton(self.display_window, text="EXTRA LARGE", variable=self.line_width_var, value="EXTRA LARGE").pack(anchor='w', padx=20)
        ttk.Button(self.display_window, text="Apply", command=self.apply_display_settings).pack(pady=10)
        ttk.Button(self.display_window, text="Back", command=self.display_window.destroy).pack(pady=5)
    ...
    def open_ticks_window(self):
        ...
        ttk.Label(ticks_window, text="SELECT HOW MANY SAMPLE NUMBER WOULD YOU LIKE:", font=('Arial', 12)).pack(pady=10)
