...
# FINAL WORKING
# PROGRAM CONVERTS TO FUNCTIONAL PROGRAMMING (NOT WORKING)
# FIX 200 ROWS EXACTLY START FROM YESTERDAY AND BACKWARD WITH SERIAL NO. AT END DATE
...
# PUT "LAST" VALUES OF PROCESS INSPECTION AFTER LCL
...
class FluctuationMonitor:
    def __init__(self, root, location, threshold=5.0, generate_csv="NO", model="60CAT0213P", selected_measurements=None):
        self.root = root
    ...
    def update_spc_graphs(self):
        ...
        try:
            ...
            for meas in self.mid_measurements:
                ...
                mean_val = model_row[self.avgs_map[meas]] if model_row is not None else full_data_prev.mean()
        ...
        try:
            ...
            if not past_data.empty:
                ...
                if not group.empty:
                    ...
                    accumulated_rows = sorted_group.tail(200)
                    count = len(accumulated_rows)
                    if count > 0:
                        recent_date = accumulated_rows.iloc[-1]['DATE'].date()
                        oldest_date = accumulated_rows.iloc[0]['DATE'].date()
                        oldest_serial = accumulated_rows.iloc[0]['S/N']
                        results.append({
            ...
            for _, row in new_df.iterrows():
                ...
                previous_pass = self.compiledFrame[(self.compiledFrame['MODEL CODE'] == model_code) & (self.compiledFrame['PASS/NG'] == 1)]
            ...
            for model in unique_models:
                group = compiledFrame[compiledFrame['MODEL CODE'] == model]
