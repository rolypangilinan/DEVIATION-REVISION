...
# UCL LCL LINE TREND
# REMOVE CURRENT DEVIATION VALUES
# TRANSFER MIN MEASUREMENTS BELOW SERIAL TEXT BOX
# TRANSFER HISTORICAL MEASUREMENTS TRENDS TO (LENGTH FROM BOUNDARY OF MID MEASUREMENT PREVIOUS LOCATION UP TO CURRENT DEVIATION LOCATION)
#           WITH UCL AND LCL
...
# WITH VERTICAL SCROLLBAR AT 1ST TKINTER
# READS NG AT 0213P
# FIX THE HEIGHT OF THE 1ST TKINTER GUI
# Fix 0212P
# FIX THE ERROR: NOT UPDATING FOR 0213P
...
class DatabaseSelection:
    def __init__(self, root):
        ...
        ttk.Label(self.scrollable_frame, text="Choose Fluctuation Threshold (%):", font=('Arial', 12)).pack(pady=10)
...
class FluctuationMonitor:
    def __init__(self, root, location, threshold=3.0, generate_csv="YES", model="60CAT0213P", selected_measurements=None):
        self.root = root
    ...
    def create_min_section(self):
        selected_in_min = [m for m in self.section_map["MIN Measurements"] if m in self.selected_measurements]
        if selected_in_min:
            min_frame = ttk.Frame(self.left_frame)
            min_frame.pack(pady=5, fill=tk.X)
            ttk.Label(min_frame, text="MIN Measurements", font=('Arial', 12, 'bold')).pack(anchor='w')
            fluct_cols = [self.measurements_map[m][3] for m in selected_in_min]
            for col in fluct_cols:
                self.create_status_row(min_frame, col)
    ...
    def zoom(self, factor):
        ...
        for frame in self.details_frame.winfo_children():
            for widget in frame.winfo_children():
                if isinstance(widget, ttk.Label) and widget.cget('text') in ["MAX Measurements", "MID Measurements"]:
                    new_size = int(12 * self.zoom_level)
                    widget.config(font=('Arial', new_size, 'bold'))

    def create_mid_trends_graphs(self):
        ...
        for meas in self.mid_measurements:
            sub_frame = ttk.Frame(self.line_graph_frame)
            ...
            ax.set_ylabel('Value')
            ax.grid(True)
            self.mid_figs[meas] = fig
            self.mid_axs[meas] = ax
            canvas = FigureCanvasTkAgg(fig, master=sub_frame)
            canvas.draw()
            canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
            self.mid_canvases[meas] = canvas

    def update_mid_trends_graphs(self):
                ...
                for meas in self.mid_measurements:
                    if meas in df.columns:
                        ...
                        if pd.isna(mean_val) or mean_val == 0:
                            continue
                        upper_limit = mean_val * 1.03
                        lower_limit = mean_val * 0.97
                        ax = self.mid_axs[meas]
                        ax.clear()
                        ax.plot(x, data.values, marker='o', linestyle='-', color='blue')
                        ...
                        if out_mask.any():
                            ...
                            out_values = data[out_mask]
                            ax.scatter(out_x, out_values, color='red', s=100, zorder=5)
                        ax.set_title(meas)
                        ...
                        self.mid_figs[meas].subplots_adjust(left=0.1, right=0.9, bottom=0.2, top=0.9)
                        self.mid_canvases[meas].draw()
        except Exception as e:
            logging.error(f"Error updating mid trends graphs: {e}")
    ...
    def on_resize(self, event):
        if event.widget == self.root:
            ...
            try:
                for meas in self.mid_measurements:
                    if meas in self.mid_figs:
                        dpi = self.mid_figs[meas].get_dpi()
                        new_width = self.line_graph_frame.winfo_width() / dpi
                        new_height = self.line_graph_frame.winfo_height() / (4 * dpi)
                        self.mid_figs[meas].set_size_inches(max(6, new_width * 0.95), max(4, new_height * 0.95))
                        self.mid_canvases[meas].draw()
            except:
                pass
# 3RD TKINTER FOCUS
    ...
    def add_to_log(self, row, count):
        log_content = f"SERIAL NO.: {row['S/N']}                     DATE: {row['DATE']}                         TIME: {row['TIME']}\n\n"
        log_content += "{:<40} {:<15} {:<15} {:<15}\n".format("PROCESS INSPECTION:", "VALUE:", "AVERAGE:", "FLUCTUATION:")
        for name, (value_col, ave_col, dev_col, fluct_col) in self.measurements_map.items():
            if name in self.selected_measurements and abs(row[fluct_col]) > self.threshold:
                ave = row[ave_col]
                dev = row[dev_col] * 100
                log_content += "{:<40} {:<15.2f} {:<15.2f} {:<15.2f}%\n".format(name, row[value_col], ave, dev)
