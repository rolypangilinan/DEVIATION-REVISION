...
# ERROR: EVERY NEW DAY FIRST RUN SERIAL WILL HAVE NO REFERENCE, AND THE DEVIATION WILL ALL BE EQUAL TO 5% (THIS MUST BE DELETED)
# ADJUST THE NEGATIVE VALUE OF BAR GRAPH (OK).
# ERROR: ADJUS AGAIN THE 1ST TKINTER WINDOW, IF I CHOOSE OTHERS THE ENTRY NUMBER IS NOT VISIBLE
# ADJUST THE 1ST TKINTER WINDOW FOR SELECTION OF DATABASE LOCATION AND FLUCTUATION THRESHOLD (3%, 5%, OTHERS RADIO BUTTON)
# CREATE 3%, 5%, OTHERS RADIO BUTTON
# FIX UPDATED CSV COPY
# ERROR CSV COPY NOT UPDATING
# COMPRESS DATA AT LINE TREND GRAPH (LINE TREND 100% WORKING)
# REMOVE PREVIOUS REF SERIAL NO.
# REMOVE USERWARNING (Explicitly setting the date format in pd.to_datetime to avoid the warning.) MUCH FASTER NOW
# ADD "WAITING FOR FILE CHANGES..." AND Modifying the file checking logic to stop the dots, show "DATA LOADED", and resume the waiting message.
# REMOVE FUTURE WARNING AND REMOVE DATAFRAME DISPLAY IN CONSOLE
# self.compiledFrame.to_csv(self.output_path, index=False, encoding='utf-8-sig')      # Save to CSV (DISABLE THIS TO CANCEL CSV OUTPUT)
# ADJUST LINE TREND GRAPH SIZE BASED ON WINDOW SIZE AND ZOOM LEVEL
# TRANSFER MIN, MID, MAX FLUCTUATION IN BETWEEN FLUCTUATION AND TREND BAR GRAPH
# WORKING (GOOD TREND AND BAR GRAPH)
# (UPDATING BUT ITS NOT THE RIGHT SIDE OF THE GRAPH)(THE MIDDLE PART IS UPDATING)
# CREATE MODEL CODE SELECTION BEFORE TKINTER DISPLAY
# CREATE A LINE TREND GRAPH FOR HISTORICAL (UPDATING BUT ITS NOT THE RIGHT SIDE OF THE GRAPH)(THE MIDDLE PART IS UPDATING)
# FUTURE WARNING REMOVED
# READS THE LATEST RUN PER MODEL AND PER DAY IT WILL COUNT HOW MANY ROWS UNTIL IT FINDS 200 ROWS
# IF THE FIRST DATE DOESN'T COMPLETE THE TOTAL OF 200 ROWS IT WILL READ ANOTHER LATEST DATE UNTIL IT COMPLETES THE 200 ROWS THEN IT WILL GET THE AVERAGE OF THOSE DATE THAT IT SEARCHES
...
#               model_summary (average) to be vlooked up at compiledFrame               compiledFrame (df) 

# CONSTANT
# --- Snip: imports & setup ---
# RUNNING AT FC1 LINE
# MONITORED 
# FIRST RUN OF PYTHON WILL HAVE NO DATA DUE TO THE LAST DATA IS ALREADY BEEN READ BY THE SYSTEM THAT'S WHY IT IS BLANK
# ERROR: HUNG ONCE, BUT STILL IT IS RUNNING (YOU NEED TO WAIT FOR THE UPDATED DATA)
# FIXED: THE "NG PRESSURE OR NO DATA ADDED" IS ALWAYS DISPLAYING AT THE CONSOLE.
# ERROR - Error processing file: 'MODEL CODE'
...
class FluctuationMonitor:
    def __init__(self, root, location, threshold=3.0):
        self.root = root
        self.root.title("Historical Measurement Trend - code1VScode2_MA2_E.py")
    ...
    def create_bar_graph(self):
        ...
        self.ax.text(1.01, self.threshold * 100, f'Threshold ({self.threshold * 100:.1f}%)', color='r', va='center', ha='left', 
    ...
    def create_line_graph(self):
        window_width = self.root.winfo_width()
        window_height = self.root.winfo_height()
        fig_width = max(8, window_width / 100 * 0.99)
