...
# SUPER FAST OUTPUT AT TKINTER IF THE GENERATE CSV IS ON OFF MODE
# CREATE A SELECTION FOR GENERATE CSV FILE 
# WITH PYTHON FILENAME UPDATE
# FIX OTHERS TOLERANCE INPUT
# ADJUST THE 1ST TKINTER GUI HEIGHT
...
class DatabaseSelection:
    def __init__(self, root):
        ...
        self.root.geometry(f"{int(self.root.winfo_screenwidth() * 0.3)}x{int(self.root.winfo_screenheight() * 0.5)}")
        ...
        ttk.Label(root, text="Generate CSV File:", font=('Arial', 12)).pack(pady=10)
        self.generate_csv_var = tk.StringVar(value="YES")
        csv_options = [
            ("YES", "YES"),
            ("NO", "NO")
        ]
        for text, val in csv_options:
            ttk.Radiobutton(root, text=text, variable=self.generate_csv_var, value=val).pack(anchor='w', padx=40)
    ...
    def on_threshold_change(self, *args):
        if self.threshold_var.get() == "others":
            ...
            self.root.geometry(f"{self.root.winfo_width()}x{int(self.root.winfo_screenheight() * 0.5)}")
            self.root.update_idletasks()
    def confirm_selection(self):
        ...
        if location:
            self.root.destroy()
            root = tk.Tk()
            app = FluctuationMonitor(root, location, threshold, generate_csv)
...
class FluctuationMonitor:
    def __init__(self, root, location, threshold=3.0, generate_csv="YES"):
        self.root = root
    ...
    def create_line_graph(self):
        ...
        self.measurement_info = [
            {"proc_name": "VOLTAGE MAX (V)", "value_col": "VOLTAGE MAX (V)", "ave_col": "AVE V_MAX PASS", "dev_col": "DEV V_MAX PASS", "fluct_col": "DEV V_MAX PASS FLUCTUATED"},
            {"proc_name": "WATTAGE MAX (W)", "value_col": "WATTAGE MAX (W)", "ave_col": "AVE WATTAGE MAX (W)", "dev_col": "DEV WATTAGE MAX (W)", "fluct_col": "DEV WATTAGE MAX (W) FLUCTUATED"},
            {"proc_name": "CLOSED PRESSURE_MAX (kPa)", "value_col": "CLOSED PRESSURE_MAX (kPa)", "ave_col": "AVE CLOSED PRESSURE_MAX (kPa)", "dev_col": "DEV CLOSED PRESSURE_MAX (kPa)", "fluct_col": "DEV CLOSED PRESSURE_MAX (kPa) FLUCTUATED"},
            {"proc_name": "VOLTAGE Middle (V)", "value_col": "VOLTAGE Middle (V)", "ave_col": "AVE VOLTAGE Middle (V)", "dev_col": "DEV VOLTAGE Middle (V)", "fluct_col": "DEV VOLTAGE Middle (V) FLUCTUATED"},
            {"proc_name": "WATTAGE Middle (W)", "value_col": "WATTAGE Middle (W)", "ave_col": "AVE WATTAGE Middle (W)", "dev_col": "DEV WATTAGE Middle (W)", "fluct_col": "DEV WATTAGE Middle (W) FLUCTUATED"},
            {"proc_name": "AMPERAGE Middle (A)", "value_col": "AMPERAGE Middle (A)", "ave_col": "AVE AMPERAGE Middle (A)", "dev_col": "DEV AMPERAGE Middle (A)", "fluct_col": "DEV AMPERAGE Middle (A) FLUCTUATED"},
            {"proc_name": "CLOSED PRESSURE Middle (kPa)", "value_col": "CLOSED PRESSURE Middle (kPa)", "ave_col": "AVE CLOSED PRESSURE Middle (kPa)", "dev_col": "DEV CLOSED PRESSURE Middle (kPa)", "fluct_col": "DEV CLOSED PRESSURE Middle (kPa) FLUCTUATED"},
            {"proc_name": "VOLTAGE MIN (V)", "value_col": "VOLTAGE MIN (V)", "ave_col": "AVE VOLTAGE MIN (V)", "dev_col": "DEV VOLTAGE MIN (V)", "fluct_col": "DEV VOLTAGE MIN (V) FLUCTUATED"},
            {"proc_name": "WATTAGE MIN (W)", "value_col": "WATTAGE MIN (W)", "ave_col": "AVE WATTAGE MIN (W)", "dev_col": "DEV WATTAGE MIN (W)", "fluct_col": "DEV WATTAGE MIN (W) FLUCTUATED"},
            {"proc_name": "CLOSED PRESSURE MIN (kPa)", "value_col": "CLOSED PRESSURE MIN (kPa)", "ave_col": "AVE CLOSED PRESSURE MIN (kPa)", "dev_col": "DEV CLOSED PRESSURE MIN (kPa)", "fluct_col": "DEV CLOSED PRESSURE MIN (kPa) FLUCTUATED"},
        ]
        self.colors = [
    ...
    def update_display(self):
        try:
            ...
            if has_fluctuation:
                self.log_text.insert(tk.END, f"SERIAL NO.: {last_row['S/N']}                     DATE: {last_row['DATE']}                         TIME: {last_row['TIME']}\n\n")
                self.log_text.insert(tk.END, f"{'PROCESS INSPECTION':<40} {'VALUE':<25} {'AVERAGE':<25} {'FLUCTUATION':<25} {'TOLERANCE':<15}\n")
                fluct_devs = []
                for info in self.measurement_info:
                    if last_row[info['fluct_col']] > self.threshold:
                        value = last_row[info['value_col']]
                        ave = last_row[info['ave_col']]
                        dev = last_row[info['dev_col']] * 100
                        tol = self.threshold * 100
                        self.log_text.insert(tk.END, f"{info['proc_name']:<40} {value:<25.2f} {ave:<25.2f} {dev:<25.2f}% {tol:<15.1f}%\n")
                        fluct_devs.append(dev)
                if fluct_devs:
                    avg_fluct = np.mean(fluct_devs)
                    std_fluct = np.std(fluct_devs)
                    self.log_text.insert(tk.END, f"\nAVERAGE: {avg_fluct:.2f}%\n")
                    self.log_text.insert(tk.END, f"DEVIATION VALUE: {std_fluct:.2f}%\n")
                    self.log_text.insert(tk.END, f"TOLERANCE: {self.threshold * 100:.1f}%\n")
    ...
    def show_focus_graph(self, measurement):
        ...
        if not df.empty:
            x = np.arange(len(df))
            ax.plot(x, df[measurement].values, color='blue', linewidth=2)
            num_ticks = min(10, len(df))
            if num_ticks > 0:
                tick_positions = np.linspace(0, len(df)-1, num_ticks, dtype=int)
                tick_labels = df['DATETIME'].iloc[tick_positions].dt.strftime('%Y-%m-%d %H:%M:%S')
