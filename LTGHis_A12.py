# ACTIVATE THE 200 ROWS BUTTON EVERY NEW START OF THE DAY AND THE START OF THE PROGRAM


# FIXED: TRIPLE COPY AT FLUCTUATION LOG AT 2ND TKINTER GUI AND Deviation_Log.txt
# UPDATE Deviation_Log.txt WHENEVER THERE IS A FLUCTUATION DETECTED
# AI
# LTG CONVERT TO FUNCTION (NO OUTPUT)
# HIDE IS ACTIVE UPON STARTING THE PROGRAM
# MULTIPLES ENTRY AT DEVIATION LOG ON NOTEPAD (CORRECTION)
# CREATE HIDE AND UNHIDE BUTTON AT 2ND TKINTER
# IF THE PROGRAM RUNS IT WILL NOT GIVE THE CORRECT SERIAL NUMBER OR END DATE FOR 200 ROWS
        # UNLESS THE CompiledPIMachine.csv IS UPDATED (MANUAL SAVE THE CSV FILE SO THAT 2ND TKINTER WILL UPDATED)
# CREATE "200 ROWS" BUTTON
# FIXED: INCLUDES ALL THE 10 PROCESS INSPECTION (MID ONLY)
# AI
# REMOVE FLUCTUATION AT 200 ROWS
# ERROR: FLUCTUATION AT NOTE PAD HAS 3 COPY PER FLUCTUATION
...
class DatabaseSelection:
    def __init__(self, root):
        ...
        # self.root.geometry(f"{int(self.root.winfo_screenwidth() * 0.3)}x{int(self.root.winfo_screenheight() * 0.75)}") # ADJUST THE WINDOW HEIGHT OF THE 1ST TKINTER GUI (1 IS THE GULL HEIGHT)
...
# 10 PROCESS INSPECTION CHECKBOXES – COMMENTED OUT
...
class FluctuationMonitor:
    def __init__(self, root, location, threshold=3.0, generate_csv="NO", model="60CAT0213P", selected_measurements=None):
        ...
        # self.root.geometry(f"{int(self.root.winfo_screenwidth() * 0.6)}x{int(self.root.winfo_screenheight() * 0.8)}")
        self.root.geometry("1000x650+350+0") # ("W+H+W+H")
        # self.root.state('zoomed')
        self.zoom_level = 1.0
        self.threshold = threshold / 100
        self.generate_csv = generate_csv
        # self.x_axis_mode = tk.StringVar(value="NONE")
        self.x_axis_mode = tk.StringVar(value="NONE")
        # self.layout_mode = tk.StringVar(value="VERTICAL")
        self.layout_mode = tk.StringVar(value="HORIZONTAL")
        ...
        self.message_labels = {}
        self.last_labels = {}
        self.ucl_text_labels = {}
        self.lcl_text_labels = {}
        self.avg_text_labels = {}
        self.last_text_labels = {}
        self.start_text_labels = {}
        self.end_text_labels = {}
        ...
        ttk.Button(button_frame, text="200 ROWS", command=self.generate_200_rows_csv).pack(side=tk.LEFT, padx=2)
        ...
        self.hide_button = ttk.Button(self.left_frame, text="Hide", command=self.toggle_hide_labels)
        self.hide_button.pack(pady=5)
        self.toggle_hide_labels()
        self.last_process_time = 0
        self.last_logged_serial = None
        # Automatically generate 200 rows CSV if it's a new day
        self.last_200_date_path = os.path.join(os.path.dirname(self.log_path), "last_200_rows_date.txt")
        try:
            with open(self.last_200_date_path, 'r') as f:
                last_date = datetime.strptime(f.read().strip(), '%Y-%m-%d').date()
        except (FileNotFoundError, ValueError):
            last_date = datetime.min.date()
        current_date = datetime.now().date()
        if current_date > last_date:
            self.generate_200_rows_csv()
            with open(self.last_200_date_path, 'w') as f:
        ...
        if self.layout_mode.get() == "HORIZONTAL":
            ...
            for i, row_meas in enumerate(meas_chunks):
                ...
                for meas in row_meas:
                    ...
                    fig = Figure(figsize=(10 * self.num_ticks / 10, 3)) #PROCESS INSPECTION HEIGHT 10, 4)
                    ...
                    self.ucl_text_labels[meas] = ttk.Label(right_sub, text="UCL:")
                    self.ucl_text_labels[meas].pack(anchor='e')
                    ucl_lab = ttk.Label(right_sub, text="N/A")
                    ucl_lab.pack(anchor='w')
                    self.ucl_labels[meas] = ucl_lab
                    self.lcl_text_labels[meas] = ttk.Label(right_sub, text="LCL:")
                    self.lcl_text_labels[meas].pack(anchor='e', pady=(10,0))
                    lcl_lab = ttk.Label(right_sub, text="N/A")
                    lcl_lab.pack(anchor='w')
                    self.lcl_labels[meas] = lcl_lab
                    self.avg_text_labels[meas] = ttk.Label(right_sub, text="Avg:")
                    self.avg_text_labels[meas].pack(anchor='e', pady=(10,0))
                    avg_lab = ttk.Label(right_sub, text="N/A")
                    avg_lab.pack(anchor='w')
                    self.avg_labels[meas] = avg_lab
                    self.last_text_labels[meas] = ttk.Label(right_sub, text="Last:")
                    self.last_text_labels[meas].pack(anchor='e', pady=(10,0))
                    last_lab = ttk.Label(right_sub, text="N/A")
                    last_lab.pack(anchor='w')
                    self.last_labels[meas] = last_lab
                    self.start_text_labels[meas] = ttk.Label(right_sub, text="Start:")
                    self.start_text_labels[meas].pack(anchor='e', pady=(10,0))
                    start_lab = ttk.Label(right_sub, text="N/A")
                    start_lab.pack(anchor='w')
                    self.start_labels[meas] = start_lab
                    self.end_text_labels[meas] = ttk.Label(right_sub, text="End:")
                    self.end_text_labels[meas].pack(anchor='e', pady=(10,0))
            ...
            for meas in self.mid_measurements:
                ...
                graph_sub = ttk.Frame(sub_frame)
                graph_sub.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
                fig = Figure(figsize=(10 * self.num_ticks / 10, 5))
                ax = fig.add_subplot(111)
                canvas = FigureCanvasTkAgg(fig, master=graph_sub)
                canvas.draw()
                canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
    ...
    def update_spc_graphs(self):
        ...
        try:
            ...
            df_prev = df_prev.sort_values('DATETIME', ascending=False).drop_duplicates(subset=['S/N'], keep='first')
            df_plot = df.tail(200)
            for meas in self.mid_measurements:
                full_data_prev = df_prev[meas].head(200)
                ...
                if self.x_axis_mode.get() == "Numerical":
                    ...
                    ax.set_xticks([])
                    ax.set_xticklabels([])
                ax.set_ylabel(meas, fontsize=12)
    ...
    def check_file_update(self):
        ...
        try:
            ...
            if current_modified > self.last_modified and current_time - self.last_process_time > 2:
    ...
    def process_and_update(self):
        ...
        try:
            ...
            if not past_data.empty:
                ...
                if not group.empty:
                    dedup_group = group.sort_values('DATETIME', ascending=False).drop_duplicates(subset=['S/N'], keep='first')
                    accumulated_rows = dedup_group.head(200)
            ...
            for _, row in new_df.iterrows():
                ...
                previous_pass = previous_pass.sort_values('DATETIME', ascending=False).drop_duplicates(subset=['S/N'], keep='first')
                previous_pass = previous_pass.head(200)
    ...
    def update_display(self):
        try:
            ...
            if has_fluctuation:
                if last_row['S/N'] != self.last_logged_serial:
                    self.add_to_log(last_row, fluctuation_count)
                    self.last_logged_serial = last_row['S/N']
...
# 3RD TKINTER FOCUS
    ...
    def show_focus_graph(self, measurement):
        ...
        ax = fig.add_subplot(111)
        ax.set_title(f'Historical Trend for {measurement}', fontsize=14)
    ...
    def open_display_window(self):
        ...
        # --- Set size and position to TOP of screen ---
        screen_width = self.root.winfo_screenwidth()
        screen_height = self.root.winfo_screenheight()
      
        width = int(screen_width * 0.3) # 30% of screen width
        height = int(screen_height * 0.8) # 80% of screen height
        x = screen_width - 1374 # 50 pixels from right edge
        y = 0 # 50 pixels from TOP of screen
        self.display_window.geometry(f"{width}x{height}+{x}+{y}")
        # self.display_window.geometry(f"{int(self.root.winfo_screenwidth() * 0.3)}x{int(self.root.winfo_screenheight() * 0.8)}") #DISPLAY SETTINGS (W X H) 0.8
        ...
        self.sample_num_var = tk.StringVar(value="")
        ...
        for name, (value_col, ave_col, dev_col, fluct_col) in self.measurements_map.items():
            if name in self.selected_measurements and row[fluct_col] > self.threshold:
                ...
                if current > upper_limit:
                    ...
                    continue
                proc_name = "CLOSED P Middle (kPa)" if name == "CLOSED PRESSURE Middle (kPa)" else name
                log_content += "{:<25} {:<15.2f} {:<5}:{:.2f}\n".format(proc_name, current, sign, limit_value)
        log_content += "\n" # empty line between entries
        # Update GUI log
        self.log_text.insert(tk.END, log_content)
        self.log_text.see(tk.END)
        # Update physical file immediately and safely
        try:
            with open(self.log_path, 'a', encoding='utf-8', buffering=1) as f: # line buffering
                f.write(log_content)
        except Exception as e:
            logging.error(f"Failed to write to Deviation_Log.txt: {e}")
    def open_ticks_window(self):
        ...
        ttk.Label(ticks_window, text="SELECT SAMPLE NUMBER:", font=('Arial', 12)).pack(pady=10)
            ...
            for meas in self.mid_measurements:
                ...
                if num_ticks > 0:
                    ...
                    if self.history_x_axis_mode.get() == "Numerical":
                        ...
                        ax.set_xlabel('Serial No.', fontsize=12)
                    ax.set_xticks(tick_positions)
                    ax.set_xticklabels(tick_labels, rotation=45, ha='right', fontsize=10)
                ax.set_ylabel(meas, fontsize=12)
                ax.grid(True, linestyle='--', alpha=0.7)
    ...
    def generate_200_rows_csv(self):
        try:
            df = self.compiledFrame[self.compiledFrame["PASS/NG"] == 1].copy()
            if self.current_model:
                df = df[df['MODEL CODE'] == self.current_model].copy()
            ...
            today = datetime.now().date()
            df = df[df['DATE'].dt.date < today]
            df = df.sort_values('DATETIME', ascending=False).drop_duplicates(subset=['S/N'], keep='first')
            fluct_cols = [self.measurements_map[m][3] for m in self.selected_measurements]
            df = df[df[fluct_cols].max(axis=1) <= self.threshold]
            df = df.sort_values('DATETIME', ascending=True).tail(200)
            df = df.sort_values('DATETIME', ascending=True)
            columns = [
                "DATE", "TIME", "MODEL CODE", "S/N", "PASS/NG"
            ] + self.selected_measurements
            df = df[columns]
            # Compute averages and UCL/LCL for mid measurements
            if "VOLTAGE Middle (V)" in df.columns:
                v_avg = df['VOLTAGE Middle (V)'].mean()
                df['VOLTAGE Middle (V) AVG'] = round(v_avg, 6)
                df['V UCL'] = round(v_avg * (1 + self.threshold), 6)
                df['V LCL'] = round(v_avg * (1 - self.threshold), 6)
            if "WATTAGE Middle (W)" in df.columns:
                w_avg = df['WATTAGE Middle (W)'].mean()
                df['WATTAGE Middle (W) AVG'] = round(w_avg, 6)
                df['W UCL'] = round(w_avg * (1 + self.threshold), 6)
                df['W LCL'] = round(w_avg * (1 - self.threshold), 6)
            if "AMPERAGE Middle (A)" in df.columns:
                a_avg = df['AMPERAGE Middle (A)'].mean()
                df['AMPERAGE Middle (A) AVG'] = round(a_avg, 6)
                df['A UCL'] = round(a_avg * (1 + self.threshold), 6)
                df['A LCL'] = round(a_avg * (1 - self.threshold), 6)
            if "CLOSED PRESSURE Middle (kPa)" in df.columns:
                cp_avg = df['CLOSED PRESSURE Middle (kPa)'].mean()
                df['CLOSED PRESSURE Middle AVG'] = round(cp_avg, 6)
                df['CP UCL'] = round(cp_avg * (1 + self.threshold), 6)
                df['CP LCL'] = round(cp_avg * (1 - self.threshold), 6)
            csv_path = os.path.join(os.path.dirname(self.output_path), "200Rows_NoFluctuate.csv")
            df.to_csv(csv_path, index=False, encoding='utf-8-sig')
            tk.messagebox.showinfo("Success", f"CSV generated at {csv_path}")
        except Exception as e:
            logging.error(f"Error generating 200 rows CSV: {e}")
            tk.messagebox.showerror("Error", "Failed to generate CSV")
    def toggle_hide_labels(self):
        self.hide_labels_var.set(not self.hide_labels_var.get())
        hidden = self.hide_labels_var.get()
        self.hide_button.config(text="Unhide" if hidden else "Hide")
        show = not hidden
        for meas in self.mid_measurements:
            if show:
                self.ucl_text_labels[meas].pack(anchor='e')
                self.ucl_labels[meas].pack(anchor='w')
                self.lcl_text_labels[meas].pack(anchor='e', pady=(10,0))
                self.lcl_labels[meas].pack(anchor='w')
                self.avg_text_labels[meas].pack(anchor='e', pady=(10,0))
                self.avg_labels[meas].pack(anchor='w')
                self.last_text_labels[meas].pack(anchor='e', pady=(10,0))
                self.last_labels[meas].pack(anchor='w')
                self.start_text_labels[meas].pack(anchor='e', pady=(10,0))
                self.start_labels[meas].pack(anchor='w')
                self.end_text_labels[meas].pack(anchor='e', pady=(10,0))
                self.end_labels[meas].pack(anchor='w')
            else:
                self.ucl_text_labels[meas].pack_forget()
                self.ucl_labels[meas].pack_forget()
                self.lcl_text_labels[meas].pack_forget()
                self.lcl_labels[meas].pack_forget()
                self.avg_text_labels[meas].pack_forget()
                self.avg_labels[meas].pack_forget()
                self.last_text_labels[meas].pack_forget()
                self.last_labels[meas].pack_forget()
                self.start_text_labels[meas].pack_forget()
                self.start_labels[meas].pack_forget()
                self.end_text_labels[meas].pack_forget()
                self.end_labels[meas].pack_forget()
